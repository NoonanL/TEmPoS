...
public class DecrementStockServlet extends HttpServlet {

    private H2Stock h2Stock;
    private H2User h2User;
    private Map<String, String> requiredParams = new HashMap<>();

    public DecrementStockServlet(){}

    public DecrementStockServlet(H2Stock h2Stock, H2User h2User){
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(inputChecker.isValid()) {
            ...
            if (h2User.isRegistered(requestUser)) {
                if (h2Stock.decrementStock(productId, branchId)) {
                    responseJson.put("response", "OK");
                    responseJson.put("error", "None.");
                } else {
                    responseJson.put("response", "false");
                    responseJson.put("error", "Failed to decrement stock listing.");
                }
